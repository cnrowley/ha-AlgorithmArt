...
async def reset_mandelbrot_zoom() -> None:
    """Tell the sidecar to delete the Mandelbrot zoom state file."""
    url = f"{SIDECAR_URL}/mandelbrot/reset"
    timeout = aiohttp.ClientTimeout(total=10)
    try:
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(url) as response:
                body = await response.json()
                _LOGGER.info("Mandelbrot reset: %s", body.get("status", "?"))
    except Exception as exc:
        _LOGGER.warning("Failed to reset Mandelbrot zoom on sidecar: %s", exc)
