...
class PhotopainterArtCoordinator(DataUpdateCoordinator):
    ...
    async def async_display_image(self, image_data: bytes) -> bool:
        ...
        The firmware accepts BMP, PNG, and JPEG.  Detect the format from the
        magic bytes and set the correct Content-Type so the device processes
        the image correctly.
        """
        ...
        _LOGGER.debug(
            "Sending image to device: %d bytes, Content-Type: %s",
            len(image_data), content_type,
        )
        ...
        try:
            async with self.session.post(
                ...
                if response.status != 200:
                    _LOGGER.error(
                        "Failed to display image: HTTP %s", response.status
                    )
                    return False
                return True
        except aiohttp.ClientError as err:
