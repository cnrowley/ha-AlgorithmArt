...
class GobanStateManager:
    ...
    def next_frame(self) -> tuple[str, int]:
        ...
        Call this on every Generate press.  Handles:
        - Advancing the move counter
        - Holding on the final position for HOLD_FINAL_FRAMES frames
        - Rolling over to the next game when complete
        """
        if not self._games:
        ...
        if move > total:
            # We're in the hold-on-final-position phase
            if hold < HOLD_FINAL_FRAMES:
                self._state["hold_counter"] = hold + 1
                self._save_state()
                sgf_text = self._read_sgf(game)
                return sgf_text, 0   # 0 = final position

            # Hold complete — start next game
            self._start_new_game()
            game  = self._game_by_id(self._state["current_game_id"])
            total = self._state.get("total_moves", 1)
            move  = self._state.get("current_move", 1)
        ...
        if next_move > total:
            self._state["current_move"] = total + 1   # trigger hold phase
            self._state["hold_counter"] = 0
        else:
    ...
    def skip_to_next_game(self) -> None:
        """Immediately skip to the next game."""
        self._start_new_game()

    def restart_current_game(self) -> None:
        """Restart from move 1 of the current game."""
        self._state["current_move"] = 1
        self._state["hold_counter"] = 0
        self._save_state()
