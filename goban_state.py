...
Game progression
----------------
Each Generate call advances the move counter by ``moves_per_frame`` (an
argument to next_frame(), default 1 — see DEFAULT_MOVES_PER_FRAME). The
scheduler's "frames per update" setting is threaded through as this value
so a single scheduled tick can jump several moves at once.
...
# Default number of moves to advance per Generate call when the caller
# doesn't specify one (1 = step-by-step). The scheduler's "frames per
# update" setting overrides this on each call — see next_frame().
DEFAULT_MOVES_PER_FRAME = 1
...
class GobanStateManager:
    ...
    def next_frame(self, moves_per_frame: int = DEFAULT_MOVES_PER_FRAME) -> tuple[str, int]:
        """Advance the state machine and return (sgf_text, move_number).

        ``moves_per_frame`` lets a caller (e.g. the scheduler's "frames per
        update" setting) jump ahead by more than one move per call instead
        of always stepping a single move at a time.
        """
        moves_per_frame = max(1, int(moves_per_frame or 1))
